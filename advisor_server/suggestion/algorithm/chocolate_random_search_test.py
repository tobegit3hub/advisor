...
class ChocolateRandomSearchAlgorithmTest(TestCase):
  def setUp(self):
    ...
    self.study = Study.create("ChocolateRandomSearchStudy",
                              study_configuration)
  ...
  def test_init(self):
    instance = ChocolateRandomSearchAlgorithm()

    self.assertTrue(isinstance(instance, AbstractSuggestionAlgorithm))

    self.assertEqual(instance.__class__, ChocolateRandomSearchAlgorithm)
